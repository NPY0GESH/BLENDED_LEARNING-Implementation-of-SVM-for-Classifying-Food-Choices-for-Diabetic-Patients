...
## Algorithm
Load Data: Read food_items_binary.csv into a pandas DataFrame.
Explore (Optional): Display initial rows and column names.
Select Features & Target: Define features (Calories, Fat, Sat. Fat, Sugars, Fiber, Protein) and the binary target ('class'). Separate into X and y.
Split Data: Use train_test_split (e.g., 70% train, 30% test, random_state=42).
Scale Features: Initialize and fit StandardScaler on training data, then transform both training and testing sets.
Tune & Train SVM:
Initialize SVC().
